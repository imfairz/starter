...
sudo pacman -S --needed --noconfirm base-devel git curl unzip bat vlc{,-plugins-all} tree tmux alacritty btop neovim fzf zig go lazygit zsh zed zoxide nvm pnpm php inter-font ttf-jetbrains-mono{,-nerd} lua-language-server docker docker-compose docker-buildx okular gwenview ast-grep
